...
function rss_scr_highlight() {
  ...
  if (rss_scr_clipwidth < rss_scr_msgwidth) {
    ...
    if (rss_scr_current == rss_scr_contents.length - 1) {
      rss_scr_current = 0
    } else {
      rss_scr_current++
    }
    scroller = setTimeout('rss_scr_changeticker()', rss_scr_delay)
  }
}

function rss_scr_start(){
  rss_scr_crosstick = document.getElementById ?
    document.getElementById('rss_scr_spancontant') : document.all.rss_scr_spancontant
  rss_scr_crosstickParent = rss_scr_crosstick.parentNode ?
    rss_scr_crosstick.parentNode : rss_scr_crosstick.parentElement
